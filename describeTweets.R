...
#'
#' Uses data sets included in the package (sentiment-orig, sentiment, sentiment-words)
...
describeTweets <- function() {
  ...
  plot3 <- qplot(1:10, 1:10, geom = "blank") + 
    theme(line = element_blank(), text = element_blank()) +
    annotation_custom(grob = tableGrob(as.data.frame(head(by_user,10)))) +
    theme_classic()

  ## list top 10 most frequent words used
  plot4 <- qplot(1:10, 1:10, geom = "blank") + 
    theme(line = element_blank(), text = element_blank()) +
    annotation_custom(grob = tableGrob(as.data.frame(head(by_word,10)))) +
    theme_classic() 
  ...
  grid.arrange(plot1,
               ...
               plot4, 
               ncol=2, 
               top = paste("Total number of tweets:", format(ntweets,big.mark=",",scientific=FALSE), 
...
## barplot(graph_data, col=colors()[c(23,89,12)] , border="white", font.axis=2, beside=T, legend=rownames(graph_data), xlab="group", font.lab=2)
  ...
  barplot(graph_data, main = "Tweets by Day of the Week, Hour",
          col=c("slategray4","steelblue4", "skyblue3", "slategray2",  "slategray1", "lightblue3"), 
          border="white", 
          beside=T, 
          legend=rownames(graph_data), 
          font.lab=1,
